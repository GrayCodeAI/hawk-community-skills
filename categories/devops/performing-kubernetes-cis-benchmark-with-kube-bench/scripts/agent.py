                    ...
                    failures.append(
                        {
                            ...
                            "remediation": test.get("remediation", test.get("Remediation", ""))[
                                :300
                            ],
    ...
    try:
        ...
        for pod in pods:
            ...
            for container in pod.get("spec", {}).get("containers", []):
                ...
                if issues:
                    findings.append(
                        {"pod": name, "container": container.get("name"), "issues": issues}
                    )
        return {
            "namespace": namespace,
            "total_pods": len(pods),
                ...
                if resource == "clusterroles":
                    for rule in item.get("rules", []):
                        ...
                        if "*" in verbs and "*" in resources:
                            ...
                            findings.append(
                                {"type": "clusterrole", "name": name, "issue": "CAN_CREATE_PODS"}
                            )
...
def main():
    ...
    b.add_argument(
        "--target", default="node", choices=["master", "controlplane", "node", "etcd", "policies"]
    )
