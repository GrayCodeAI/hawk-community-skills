...
def get_running_containers():
    ...
    try:
        result = subprocess.check_output(
            ["docker", "ps", "--format", "{{json .}}"], text=True, errors="replace", timeout=10
        )
        containers = []
...
def audit_container(container_id):
    ...
    config.get("Name", container_id)

    if host_config.get("Privileged"):
        findings.append(
            {"check": "CIS 5.4", "issue": "Container runs privileged", "severity": "CRITICAL"}
        )
    if host_config.get("NetworkMode") == "host":
        findings.append({"check": "CIS 5.10", "issue": "Uses host network", "severity": "HIGH"})
    if host_config.get("PidMode") == "host":
        findings.append(
            {"check": "CIS 5.15", "issue": "Shares host PID namespace", "severity": "HIGH"}
        )
    if host_config.get("IpcMode") == "host":
        findings.append(
            {"check": "CIS 5.16", "issue": "Shares host IPC namespace", "severity": "HIGH"}
        )
    ...
    if "SYS_ADMIN" in cap_add:
        findings.append(
            {"check": "CIS 5.3", "issue": "SYS_ADMIN capability added", "severity": "CRITICAL"}
        )
    if "NET_ADMIN" in cap_add:
        findings.append(
            {"check": "CIS 5.3", "issue": "NET_ADMIN capability added", "severity": "HIGH"}
        )

    if not host_config.get("ReadonlyRootfs"):
        findings.append(
            {"check": "CIS 5.12", "issue": "Root filesystem not read-only", "severity": "MEDIUM"}
        )
    ...
    for mount in mounts:
        ...
        if src in sensitive:
            findings.append(
                {"check": "CIS 5.5", "issue": f"Sensitive host mount: {src}", "severity": "HIGH"}
            )
...
def main():
    parser = argparse.ArgumentParser(description="Audit Docker containers against CIS benchmarks")
    ...
    critical = sum(
        1 for a in report["audits"] for f in a["findings"] if f["severity"] == "CRITICAL"
    )
