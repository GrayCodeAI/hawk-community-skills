...
def check_container_configs():
    ...
    try:
        result = subprocess.run(
            ["docker", "ps", "--format", "{{json .}}"], capture_output=True, text=True, timeout=30
        )
...
def _check_container_security(inspect, name):
    ...
    if host_config.get("PidMode") == "host":
        issues.append(
            {"severity": "HIGH", "check": "5.15", "finding": "Container shares host PID namespace"}
        )
    ...
    if config.get("User", "") in ("", "root", "0"):
        issues.append(
            {"severity": "MEDIUM", "check": "4.1", "finding": "Container running as root user"}
        )
