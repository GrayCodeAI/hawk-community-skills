...
def run_codeql_query(repo_path: str, language: str, database_path: str) -> ScanResult:
    ...
    try:
        proc = subprocess.run(cmd, cwd=repo_path, capture_output=True, text=True, timeout=1200)
        result.exit_code = proc.returncode
        ...
        result.error_message = (
            "codeql binary not found. Install from https://github.com/github/codeql-cli-binaries"
        )
        result.exit_code = -1
        return result
...
def evaluate_quality_gate(findings: list, severity_threshold: str) -> dict:
    ...
    blocking_findings = [
        f for f in findings if SEVERITY_ORDER.get(f.severity.lower(), 5) <= threshold_level
    ]
...
def main():
    ...
    parser.add_argument(
        "--sarif-only", nargs="*", help="Only parse existing SARIF files instead of running scans"
    )
    ...
    if args.sarif_only:
        ...
        if not args.skip_semgrep:
            ...
            semgrep_result = run_semgrep(
                repo_path, config=args.semgrep_config, extra_configs=args.semgrep_extra_configs
            )
