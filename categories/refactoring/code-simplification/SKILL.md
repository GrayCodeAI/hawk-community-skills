...
description: "Reduces complexity without changing behavior. Refactoring patterns and simplification strategies."
license: MIT
tags: [refactoring, simplification, cleanup, complexity]
domain: general
version: 1.0
author: graycode
---
...
label := "Active"
if isNew { label = "New" } else if isUpdated { label = "Updated" } else if isArchived { label = "Archived" }

// CLEAR: Readable function
...
| Boolean parameter flags | `DoThing(true, false, true)` | Replace with options structs or separate functions |
...
## Go-Specific Simplification Examples

```go
// SIMPLIFY: Unnecessary error wrapping
// Before
...
// SIMPLIFY: Manual loop to filter
// Before
...
// After
activeUsers := slices.Filter(users, func(u *User) bool { return u.IsActive })
```
...
| "The types make it self-documenting" | Types document structure, not intent. A well-named function explains *why* better. |
| "This abstraction might be useful later" | Don't preserve speculative abstractions. If it's not used now, it's complexity without value. |
