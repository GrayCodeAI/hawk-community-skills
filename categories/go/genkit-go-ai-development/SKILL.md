---
name: genkit-go-ai-development
description: "Builds AI-powered applications in Go using the Genkit AI SDK, covering generation, prompts, tool calling, streaming, flows, agents, middleware, and model providers."
license: Apache-2.0
tags:
- go
- ai
...
-   Agents: defining/serving an agent, running turns, and client- vs server-managed state (start here).
-   Sessions & persistence: session stores (`localstore.NewInMemorySessionStore`/`NewFileSessionStore`) and snapshots.
...
-   Working with state: typed custom session state, streamed as JSON patches.
-   Artifacts: producing and reading named deliverables.
-   Multi-agent orchestration: delegating to sub-agents.
-   Advanced custom agents: `DefineCustomAgent` for full turn control.
-   Deploying agents: serving agents over HTTP with `genkit.Handler`.
