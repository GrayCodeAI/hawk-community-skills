---
name: ai-writing-pattern-removal
description: "Edits drafts to remove machine-written patterns while preserving facts, voice, and format, or detects AI-writing patterns without rewriting."
license: MIT
tags:
- editing
- prose
...
3. **Load slop-catalog.md** — the word, phrase, and pattern cues both modes scan for.
4. **Load the mode's contract**: edit.md for a rewrite, detect.md for a report. Each carries its own steps, output template, and MUST-NOT list.
5. **Load editing-principles.md** when editing — the rules for preserving voice and making the smallest useful change.
6. **Load self-check.md** before returning an edit, run the checks directly, fix each failure, and run them again.
