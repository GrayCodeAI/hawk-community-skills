---
name: feature-documentation-drafting
description: "Use when drafting net-new docs for a feature or launch, grounded in product intent and actual code, rather than fixing existing docs tickets or restructuring pages."
license: Apache-2.0
tags:
- documentation
- writing
...
- **Write for timelessness.** Prefer documenting what exists now over promising future features. See reference/common-pitfalls.md.
- **Keep it concise and avoid redundancy.** See reference/common-pitfalls.md.
- **Prefer paragraphs over single-item lists.** See reference/common-pitfalls.md.
...
- Search `apps/docs/WORD_LIST.md` when introducing or reviewing technical terms, UI actions, abbreviations, and potentially ambiguous language during drafting. This targeted search supplements, but does not replace, the full-file compliance check in Phase 2.5.
...
Re-read `apps/docs/CONTRIBUTING.md` and `apps/docs/WORD_LIST.md` in full before handoff, not just the sections searched during drafting. When a dedicated style guide lands in the repo, extend this checklist to cover it too.
...
- Then run `review-the-docs` local self-review (lint/build/classify).
...
- Before opening the PR, run `review-the-docs` local self-review: `pnpm lint:mdx`, `pnpm build:guides-markdown` where applicable, and anchor checks per reference/drafting-mechanics.md.

## Additional resources

- Style / terminology: `apps/docs/CONTRIBUTING.md`, `apps/docs/WORD_LIST.md`, reference/style-fallback.md
- Common pitfalls to avoid: reference/common-pitfalls.md
- Drafting mechanics: reference/drafting-mechanics.md
- Content-type gate detail: reference/content-type-gate.md
- Existing-page restructure/clarity: `edit-the-docs`
- "Write the docs" checklist (Draft stage): `pm-the-docs`'s reference/write-the-docs-checklist.md
- Cross-repo product lookup: `pm-the-docs` → universe-lookup
- Runnable verification: `test-the-docs`
- Docs-app architecture/placement: `ask-the-docs`, [`audit-docs-ia`](https://github.com/supabase/docs-agent-skills/blob/main/.claude/skills/audit-docs-ia/SKILL.md)
