---
name: cloud-document-editing
description: "Create, read, and edit cloud documents through a suite CLI: manage blocks, headings, tables, and document content. Use when a user wants to work with shared documents."
license: MIT
tags:
- documentation
- docs
...
- **草稿初始化、解析与统计 — `+script`**：支持解析文档 URL / token 与本地 XML，统计字数并返回字符诊断；不支持 Markdown 输入。
- **历史版本 — `+history-list` / `+history-revert` / `+history-revert-status`**：查询、回滚文档历史版本或检查回滚任务状态。
...
- **画板 — `画板工作流`**：创建或更新画板时先读取工作流；更新已有画板必须复用现有 token，禁止新建空白画板；使用 `whiteboard +update` 写入。
- **思维笔记 — `mindnotes`**：已有思维笔记走 `思维笔记链路`；新建思维笔记走 `lark-doc-whiteboard`。
