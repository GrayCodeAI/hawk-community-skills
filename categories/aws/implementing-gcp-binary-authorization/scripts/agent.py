...
def get_binauthz_policy(project):
    ...
    return run_gcloud(["container", "binauthz", "policy", "export", "--project", project])


def list_attestors(project):
    """List all attestors in the project."""
    return run_gcloud(["container", "binauthz", "attestors", "list", "--project", project])
...
def audit_policy(policy):
    ...
    if isinstance(policy, dict) and "error" in policy:
        return [
            {"issue": "Cannot retrieve policy", "severity": "CRITICAL", "detail": policy["error"]}
        ]
    ...
    if eval_mode == "REQUIRE_ATTESTATION" and not attestors:
        findings.append(
            {"issue": "Attestation required but no attestors configured", "severity": "CRITICAL"}
        )
    ...
    for cluster, rule in cluster_rules.items():
        if rule.get("evaluationMode") == "ALWAYS_ALLOW":
            findings.append({"issue": f"Cluster {cluster} allows all images", "severity": "HIGH"})
...
def generate_policy(project, attestors, allowed_patterns=None):
    ...
    policy = {
        ...
        "defaultAdmissionRule": {
            ...
            "requireAttestationsBy": [f"projects/{project}/attestors/{a}" for a in attestors],
        },
...
def check_cv_status(project, cluster, zone):
    ...
    result = run_gcloud(
        ["container", "clusters", "describe", cluster, "--zone", zone, "--project", project]
    )
...
def main():
    ...
    report = {"generated_at": datetime.utcnow().isoformat(), "project": args.project, "results": {}}
