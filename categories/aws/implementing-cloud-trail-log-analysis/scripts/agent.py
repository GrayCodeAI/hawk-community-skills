...
def lookup_events(client, event_name=None, hours=24, max_results=50):
    ...
    kwargs = {"StartTime": start_time, "MaxResults": max_results, "LookupAttributes": []}
    if event_name:
...
def detect_suspicious_activity(client, hours=24):
    ...
    for event_name, description in SUSPICIOUS_EVENTS.items():
        ...
        for e in events:
            ...
            severity = (
                ...
                if event_name in ["CreateUser", "CreateAccessKey", "AttachUserPolicy"]
                else "MEDIUM"
            )
            detections.append(
                {
    ...
    return sorted(
        detections, key=lambda x: {"CRITICAL": 0, "HIGH": 1, "MEDIUM": 2}.get(x["severity"], 3)
    )
...
def detect_unauthorized_regions(client, authorized_regions, hours=24):
    ...
    unauthorized = [
        e
        for e in events
        if e.get("region") and e["region"] not in authorized_regions and not e.get("error")
    ]
    return unauthorized
