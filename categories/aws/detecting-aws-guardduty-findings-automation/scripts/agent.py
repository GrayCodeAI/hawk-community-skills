...
def list_findings(session, detector_id, severity_min=4.0, max_results=50):
    ...
    details = gd.get_findings(DetectorId=detector_id, FindingIds=finding_ids)["Findings"]
...
def check_member_accounts(session, detector_id):
    ...
    gd = session.client("guardduty")
    members = gd.list_members(DetectorId=detector_id, OnlyAssociated="true").get("Members", [])
...
def main():
    ...
    parser.add_argument(
        "--min-severity", type=float, default=4.0, help="Minimum severity threshold (default: 4.0)"
    )
