...
def assess_authentication_methods(auth_config):
    ...
    if isinstance(methods, dict):
        ...
        if not mfa_enabled:
            findings.append({"control": "MFA", "status": "MISSING", "severity": "CRITICAL"})
        ...
        if not fido2:
            findings.append(
                {"control": "FIDO2/WebAuthn", "status": "NOT_DEPLOYED", "severity": "MEDIUM"}
            )
        if not passwordless:
            findings.append(
                {
        ...
        if not sso:
            findings.append({"control": "SSO", "status": "NOT_DEPLOYED", "severity": "HIGH"})
    return findings
...
def assess_conditional_access(policies_path):
    ...
    findings = []
    required_signals = ["device_compliance", "location", "risk_level", "application", "user_group"]
    covered_signals = set()
...
def assess_identity_maturity(config):
    ...
    categories = {
        "authentication": {
            "checks": ["mfa_enforced", "phishing_resistant_mfa", "passwordless", "continuous_auth"],
        },
        "identity_stores": {
            "checks": [
                "centralized_idp",
...
def analyze_auth_events(events_path):
    ...
    return {
        ...
        "password_only_rate": round(by_method.get("password", 0) / len(items) * 100, 1)
        if items
        else 0,
    }
...
def main():
    ...
    parser.add_argument(
        "--action", choices=["auth", "policies", "maturity", "events", "full"], default="full"
    )
