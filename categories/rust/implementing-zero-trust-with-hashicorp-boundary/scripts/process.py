...
class BoundaryTerraformGenerator:
    ...
    def add_scope(
        self, name: str, description: str, scope_type: str = "project", parent: str = "global"
    ) -> BoundaryScope:
        scope = BoundaryScope(
            name=name, description=description, scope_type=scope_type, parent_scope_id=parent
        )
        self.scopes.append(scope)
        return scope

    def add_target(
        self, name: str, description: str, target_type: str, port: int, hosts: list, **kwargs
    ) -> BoundaryTarget:
...
''')
    ...
    def generate_roles(self) -> str:
        ...
        for role in self.roles:
            ...
            principals = ",\n    ".join(
                f"boundary_managed_group.{g.replace('-', '_')}.id" for g in role.principal_groups
            )
            blocks.append(f'''
...
def main():
    ...
    gen.add_target(
        "redis-cache", "Production Redis cache", "tcp", 6379, ["10.0.3.30"], session_max_seconds=900
    )
