...
class WebSocketSecurityAgent:
    ...
    async def _connect(self, headers=None, origin=None, timeout=5):
        ...
        if origin:
            extra["origin"] = origin
        try:
            return await asyncio.wait_for(websockets.connect(self.ws_url, **extra), timeout=timeout)
        except Exception:
            return None
        ...
        for payload in INJECTION_PAYLOADS:
            try:
                ...
                if "error" not in resp.lower() and len(resp) > 10:
                    results.append(
                        {"payload": payload[:80], "response": resp[:200], "potential_issue": True}
                    )
                    self.findings.append(
                        {
                            "severity": "medium",
    ...
    def check_upgrade_headers(self):
        ...
        try:
            ...
            for issue in issues:
                self.findings.append(
                    {"severity": "low", "type": "Missing Security Header", "detail": issue}
                )
