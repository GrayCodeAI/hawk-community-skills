...
def assess_compliance(assessment_csv):
    ...
    gaps = [
        row
        for row in rows
        if row.get("status", "").lower() in ("fail", "non-compliant", "no", "not met", "gap")
    ]
    return {
...
def assess_esp_security(firewall_csv):
    ...
    with open(firewall_csv, encoding="utf-8", errors="replace") as f:
        reader = csv.DictReader(f)
        rules = list(reader)
    findings = []
    for rule in rules:
        ...
        if action == "allow" and rule.get("protocol", "").lower() in ("any", "all"):
            findings.append(
                {"rule": rule.get("id", ""), "issue": "ALLOW_ANY_PROTOCOL", "severity": "HIGH"}
            )
