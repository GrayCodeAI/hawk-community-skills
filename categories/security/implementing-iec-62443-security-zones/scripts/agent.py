...
def audit_zone_architecture(zones_path):
    ...
    for zone in zones:
        ...
        if not sl_target:
            findings.append({"zone": zone_name, "issue": "No SL-T defined", "severity": "CRITICAL"})
        ...
        if not zone.get("assets"):
            findings.append(
                {"zone": zone_name, "issue": "No assets documented", "severity": "MEDIUM"}
            )
    ...
    for conduit in conduits:
        ...
        if conduit.get("allows_remote_access", False) and not conduit.get("requires_mfa", False):
            findings.append(
                {
                    "conduit": f"{src} -> {dst}",
...
def main():
    ...
    parser.add_argument("--action", choices=["audit", "template", "sl-map", "full"], default="full")
