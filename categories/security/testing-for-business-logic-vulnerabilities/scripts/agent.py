...
def test_price_manipulation(base_url, token, cart_endpoint="/api/cart/add"):
    ...
    for tc in test_cases:
        try:
            resp = requests.post(url, headers=headers, json=tc["payload"], timeout=10, verify=True)
...
def test_coupon_reuse(base_url, token, coupon_endpoint="/api/cart/apply-coupon", code="DISCOUNT50"):
    ...
    for _i in range(5):
        try:
            resp = requests.post(
                url, headers=headers, json={"coupon_code": code}, timeout=10, verify=True
            )
            if resp.status_code in (200, 201):
                success_count += 1
        except requests.RequestException:
...
def test_self_referral(
    ...
    try:
        resp = requests.post(
            url, headers=headers, json={"referral_email": email}, timeout=10, verify=True
        )
        if resp.status_code in (200, 201):
...
def main():
    ...
    findings.extend(
        test_race_condition(
            args.base_url, args.token, args.cart_endpoint, {"coupon_code": args.coupon_code}
        )
    )
