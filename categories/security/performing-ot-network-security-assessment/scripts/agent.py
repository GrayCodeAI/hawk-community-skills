...
def assess_asset_inventory(csv_file):
    ...
    for asset in assets:
        ...
        if asset.get("end_of_life", "").lower() in ("yes", "true"):
            findings.append(
                {"asset": asset.get("name", ""), "issue": "END_OF_LIFE", "severity": "HIGH"}
            )
        if not firmware:
            findings.append(
                {"asset": asset.get("name", ""), "issue": "UNKNOWN_FIRMWARE", "severity": "MEDIUM"}
            )
    return {
...
def assess_network_segmentation(csv_file):
    ...
    return {
        ...
        "dmz_rules": sum(
            1
            for r in rules
            if "dmz" in r.get("src_zone", "").lower() or "dmz" in r.get("dst_zone", "").lower()
        ),
    }
