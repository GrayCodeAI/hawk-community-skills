...
def calculate_criticality_score(asset):
    ...
    weighted_score = sum(scores[factor] * weight for factor, weight in CRITICALITY_WEIGHTS.items())
...
def run_audit(args):
    ...
    if args.inventory:
        ...
        for s in scored[:20]:
            print(
                f"  Tier {s['tier']} ({s['tier_name']}): {s['asset']} — score {s['weighted_score']}"
            )
...
def main():
    ...
    parser.add_argument(
        "--asset-tier", type=int, choices=[1, 2, 3, 4, 5], help="Asset criticality tier (1=highest)"
    )
