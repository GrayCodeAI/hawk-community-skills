...
class ThreatFeed:
    ...
    def __init__(
        self, name: str, feed_type: str, url: str, polling_interval: int = 3600, api_key: str = ""
    ):
        self.name = name
...
class EnrichmentPipeline:
    ...
    def simulate_correlation(self, events: list) -> list:
        ...
        domain_iocs = {
            r["domain"]: r for r in self.kv_store.get("domain_intel", []) if "domain" in r
        }
...
if __name__ == "__main__":
    ...
    otx_feed.add_indicator(
        ThreatIndicator("203.0.113.50", "ip", "OTX", "C2", 85, "Known C2 server", "high")
    )
    otx_feed.add_indicator(
        ThreatIndicator("198.51.100.25", "ip", "OTX", "Scanner", 60, "Port scanner", "medium")
    )
    otx_feed.add_indicator(
        ThreatIndicator(
            "evil-domain.com", "domain", "OTX", "Phishing", 92, "Phishing domain", "critical"
        )
    )

    abuse_feed = ThreatFeed("AbuseIPDB", "csv", "https://api.abuseipdb.com/api/v2/blacklist")
    abuse_feed.add_indicator(
        ThreatIndicator("203.0.113.50", "ip", "AbuseIPDB", "C2", 90, "Reported C2", "high")
    )
    abuse_feed.add_indicator(
        ThreatIndicator(
            "192.0.2.100", "ip", "AbuseIPDB", "Brute Force", 75, "SSH brute forcer", "medium"
        )
    )
