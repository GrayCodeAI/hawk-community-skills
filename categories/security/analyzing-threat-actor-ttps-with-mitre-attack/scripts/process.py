...
class ATTACKAnalyzer:
    ...
    def compare_groups(self, group_names: list) -> dict:
        ...
        for name, techs in group_techs.items():
            ...
            comparison["per_group"][name] = {
                ...
                "overlap_percentage": round(len(techs.intersection(others)) / len(techs) * 100, 1)
                if techs
                else 0,
            }
    ...
    def gap_analysis(self, group_name: str, detected_techniques: set) -> dict:
        ...
        analysis = {
            ...
            "gaps": len(gaps),
            "coverage_percentage": round(len(covered) / len(actor_tech_ids) * 100, 1)
            if actor_tech_ids
            else 0,
...
def main():
    parser = argparse.ArgumentParser(description="MITRE ATT&CK Threat Actor TTP Analyzer")
