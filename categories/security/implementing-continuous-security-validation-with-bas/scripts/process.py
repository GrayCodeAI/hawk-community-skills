...
CONTROL_TECHNIQUE_MAP = {
    ...
    "ngfw_proxy": ["T1071.001", "T1071.004", "T1048.001", "T1048.003", "T1572", "T1090"],
...
def print_summary(scores_df, gaps_df):
    ...
    if len(gaps_df) > 0:
        ...
        for _, row in gaps_df.head(10).iterrows():
            print(f"  {row['technique_id']}: {row['technique_name']} ({row['miss_count']} misses)")
