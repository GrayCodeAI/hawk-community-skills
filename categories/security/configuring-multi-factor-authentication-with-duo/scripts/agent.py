...
class DuoAdminClient:
    ...
    def _get(self, endpoint, params=None):
        ...
        resp = requests.get(f"https://{self.host}{endpoint}", headers=headers, params=params)
...
def audit_mfa_coverage(users_data):
    ...
    no_device = [
        u["username"] for u in users if not u.get("phones") and u.get("status") == "active"
    ]
    return {
        "total_users": total,
        "enrolled": enrolled,
        ...
        "findings": [
            ...
            {"severity": "MEDIUM", "issue": f"{len(no_device)} users without MFA device"}
            if no_device
            else None,
        ],
    }
