...
class VulnerabilityEnricher:
    ...
    def get_nvd_data(self, cve_id: str) -> dict:
        ...
        try:
            response = self.session.get(self.NVD_API, params={"cveId": cve_id}, timeout=30)
    ...
    def get_epss_score(self, cve_id: str) -> dict:
        ...
        try:
            response = self.session.get(self.EPSS_API, params={"cve": cve_id}, timeout=15)
            if response.status_code == 200:
                data = response.json()
                if data.get("data"):
                    entry = data["data"][0]
...
class VulnerabilityPrioritizer:
    ...
    def assign_sla(self, priority_score: float, cvss_score: float, in_kev: bool = False) -> dict:
    ...
    def prioritize(self, vulnerabilities: list) -> pd.DataFrame:
        ...
        for vuln in vulnerabilities:
            ...
            sla = self.assign_sla(
                score, float(vuln.get("cvss_base_score", 0)), vuln.get("in_cisa_kev", False)
            )
...
def main():
    ...
    if args.command == "score":
        ...
        sla = prioritizer.assign_sla(
            priority, float(result.get("cvss_base_score", 0)), result.get("in_cisa_kev", False)
        )
