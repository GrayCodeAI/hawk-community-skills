...
class SOAPSecurityTester:
    ...
    def _send_soap(self, body_xml, soap_action="", timeout=10):
        ...
        return requests.post(self.endpoint_url, data=body_xml, headers=headers, timeout=timeout)
    ...
    def test_sql_injection(self, operation_name, soap_action=""):
        ...
        for payload in sqli_payloads:
            ...
            try:
                ...
                results.append(
                    {"payload": payload, "sql_error": error_found, "status": resp.status_code}
                )
            ...
            for j, other in enumerate(self.operations):
                ...
                try:
                    ...
                    if resp.status_code == 200 and "Fault" not in resp.text:
                        ...
                        results.append(
                            {"op": op["name"], "spoofed": other["action"], "accepted": True}
                        )
                except requests.RequestException:
                    continue
        return results
    ...
    def test_ws_security_bypass(self):
        ...
        try:
            ...
            if accepted:
                self.findings.append(
                    {"severity": "CRITICAL", "type": "WS-Security Bypass", "operation": op["name"]}
                )
