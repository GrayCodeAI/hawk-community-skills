        ...
        slots.append(
            {
                ...
                "slot_description": slot.slot_description.strip()
...
def list_objects(lib, token_label, pin):
    ...
    with token.open(user_pin=pin) as session:
        ...
        for obj in session.get_objects():
            obj_info = {
                "object_class": obj.object_class.name
            ...
            if hasattr(obj, "key_type"):
                obj_info["key_type"] = (
                    obj.key_type.name if hasattr(obj.key_type, "name") else str(obj.key_type)
                )
...
def full_audit(lib, token_label, pin):
    ...
    weak_keys = [
        o for o in objects if o.get("key_type") == "RSA" and (o.get("key_length") or 2048) < 2048
    ]
    fips_mechs = {
        "RSA_PKCS",
        "SHA256_RSA_PKCS",
