...
class IoTSecurityAgent:
    ...
    def analyze_firmware(self, firmware_path):
        ...
        scan_result = subprocess.run(["binwalk", str(fw_path)], capture_output=True, text=True)
        ...
        creds_found = []
        for _root, _dirs, _files in (extract_dir).rglob("*") if extract_dir.exists() else []:
            pass
    ...
    def capture_traffic(self, interface="eth0", duration=30):
        ...
        if pcap_path.exists():
            stats = subprocess.run(["capinfos", str(pcap_path)], capture_output=True, text=True)
