...
class OAuth2Auditor:
    ...
    SECURE_AUTH_METHODS = ["private_key_jwt", "tls_client_auth", "self_signed_tls_client_auth"]
    ...
    def _audit_token_endpoint_auth(self):
        ...
        has_secure = any(
            m in self.SECURE_AUTH_METHODS for m in self.config.token_endpoint_auth_methods
        )
        if has_secure:
            self.findings.append(
                AuditFinding(
