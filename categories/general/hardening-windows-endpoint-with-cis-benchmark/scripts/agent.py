...
def check_password_policy():
    ...
    for line in result.splitlines():
        ...
        if "Maximum password age" in line:
            ...
            if val == "Unlimited":
                findings.append(
                    {"check": "1.1.2", "issue": "No max password age", "severity": "MEDIUM"}
                )
...
def check_audit_policy():
    ...
    for subcategory, _expected in required_audits.items():
        if subcategory in result:
            for line in result.splitlines():
                if subcategory in line and "No Auditing" in line:
                    findings.append(
                        {
                            "check": "17.x",
...
def check_security_options():
    ...
    for path, name, expected, cis_id, desc in checks:
        ...
        if result.strip().isdigit() and int(result.strip()) != expected:
            findings.append(
                {"check": cis_id, "issue": desc, "current": result.strip(), "severity": "HIGH"}
            )
    return findings
...
def main():
    parser = argparse.ArgumentParser(description="Audit Windows endpoint against CIS Benchmark")
