    ...
    for i, char in enumerate(domain):
        for latin, lookalikes in homograph_map.items():
            if char in lookalikes:
                findings.append(
                    {"position": i, "character": char, "looks_like": latin, "type": "homograph"}
                )
    return findings
        ...
        if 0 < dist <= 2:
            ...
            if len(domain_base) != len(known_base):
                ...
                for i, (a, b) in enumerate(zip(domain_base, known_base)):
                    if a != b:
                        if (
                            i > 0 and domain_base[i - 1 : i + 1] == known_base[i : i - 2 : -1]
                            if i > 0
                            else False
                        ):
...
def detect_urgency(text: str) -> list:
    ...
    for pattern in URGENCY_KEYWORDS:
        ...
        if matches:
            findings.append({"pattern": pattern, "matches": matches, "count": len(matches)})
    return findings
...
def detect_impersonation(display_name: str, vip_names: list = None) -> list:
    ...
    for vip in vip_names:
        if vip.lower() in name_lower:
            findings.append({"matched_vip": vip, "display_name": display_name, "confidence": 0.8})
...
def format_analysis_report(analysis: EmailAnalysis) -> str:
    ...
    lines.append("=" * 60)
    lines.append(
        f"  Risk Level: {analysis.risk_level.upper()} (Score: {analysis.risk_score:.0f}/100)"
    )
    lines.append(
