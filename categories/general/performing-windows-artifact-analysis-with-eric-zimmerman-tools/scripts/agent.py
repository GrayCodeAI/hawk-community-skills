...
class EZToolsAgent:
    ...
    def parse_mft(self, mft_path):
        ...
        if "error" in result:
            return result
        return self._parse_csv(csv_out, "MFT", time_cols=["Created0x10", "LastModified0x10"])
    ...
    def parse_prefetch(self, prefetch_dir):
        ...
        result = self._run_tool(
            "PECmd.exe",
            ["-d", prefetch_dir, "--csv", str(self.output_dir), "--csvf", "prefetch_output.csv"],
        )
        if "error" in result:
            return result
        return self._parse_csv(csv_out, "Prefetch", time_cols=["LastRun", "PreviousRun0"])

    def parse_lnk_files(self, lnk_dir):
        """Parse LNK shortcut files using LECmd."""
        csv_out = self.output_dir / "lnk_output.csv"
        result = self._run_tool(
            "LECmd.exe", ["-d", lnk_dir, "--csv", str(self.output_dir), "--csvf", "lnk_output.csv"]
        )
        if "error" in result:
            return result
        return self._parse_csv(csv_out, "LNK", time_cols=["TargetCreated", "TargetModified"])
    ...
    def parse_jump_lists(self, jl_dir):
        ...
        result = self._run_tool(
            "JLECmd.exe",
            ["-d", jl_dir, "--csv", str(self.output_dir), "--csvf", "jumplist_output.csv"],
        )
        if "error" in result:
            return result
        return self._parse_csv(csv_out, "JumpList", time_cols=["TargetCreated", "TargetModified"])
    ...
    def parse_shellbags(self, registry_hive):
        ...
        result = self._run_tool(
            "SBECmd.exe",
            ["-d", registry_hive, "--csv", str(self.output_dir), "--csvf", "shellbags_output.csv"],
        )
        if "error" in result:
            return result
        return self._parse_csv(csv_out, "ShellBag", time_cols=["LastWriteTime", "FirstExplored"])
                                ...
                                self.timeline.append(
                                    {
                                        ...
                                        "details": {k: v for k, v in row.items() if v and k != tc},
                                    }
                                )
                                break
