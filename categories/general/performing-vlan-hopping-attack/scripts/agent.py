...
try:
    from scapy.all import ICMP, IP, LLC, SNAP, Dot1Q, Ether, Raw, conf, get_if_hwaddr, sendp, sniff
except ImportError:
...
def listen_for_dtp(iface="eth0", timeout=30):
    ...
    sniff(
        iface=iface, filter="ether dst 01:00:0c:cc:cc:cc", prn=dtp_handler, timeout=timeout, store=0
    )
    return {
...
def create_vlan_interface(iface, vlan_id, ip_addr):
    ...
    subprocess.run(["ip", "addr", "add", f"{ip_addr}/24", "dev", vlan_iface], capture_output=True)
...
def cleanup_vlan_interfaces(iface, vlan_ids):
    ...
    for vid in vlan_ids:
        ...
        result = subprocess.run(["ip", "link", "del", vlan_iface], capture_output=True)
