...
class YaraRuleBuilder:
    ...
    def analyze_sample(self, filepath):
        ...
        ascii_strings = [s.decode("ascii") for s in re.findall(rb"[\x20-\x7e]{8,}", data)]
                ...
                if hasattr(pe, "DIRECTORY_ENTRY_IMPORT"):
                    for entry in pe.DIRECTORY_ENTRY_IMPORT:
                        for imp in entry.imports:
                            if imp.name:
                                self.imports.append(imp.name.decode("utf-8", errors="replace"))
            except Exception:
                pass
    ...
    def generate_rule(self, name, author="analyst", description=""):
        ...
        sus_imports = [
            i
            for i in self.imports
            if i in ["VirtualAlloc", "VirtualAllocEx", "WriteProcessMemory", "CreateRemoteThread"]
        ]
    ...
    def test_rule(self, rule_path_or_text, sample_dir, clean_dir=None):
            ...
            if f.is_file():
                try:
                    ...
                    if matches:
                        ...
                        results["details"].append({"file": str(f), "result": "FALSE_NEGATIVE"})
                except Exception:
                    pass
                ...
                if f.is_file():
                    try:
                        ...
                        if matches:
                            ...
                            results["details"].append({"file": str(f), "result": "FALSE_POSITIVE"})
                        else:
        ...
        if total_samples > 0:
            results["detection_rate"] = round(results["true_positives"] / total_samples * 100, 2)

        total_clean = results["false_positives"] + results["true_negatives"]
        if total_clean > 0:
            results["fp_rate"] = round(results["false_positives"] / total_clean * 100, 4)
...
def main():
    ...
    parser.add_argument("--generate", action="store_true", help="Generate rule from analysis")
