...
class YaraRuleDeveloper:
    ...
    def extract_strings(self, sample_path, min_length=8, max_strings=30):
        ...
        unique_ascii = list(set(s.decode("ascii", errors="ignore") for s in ascii_strings))
        unique_wide = list(set(s.decode("utf-16-le", errors="ignore") for s in wide_strings))
    ...
    def generate_rule(self, rule_name, sample_path, description="", tags=None, author="auto"):
...
"""
        ...
        self.rules.append(
            {"name": rule_name, "path": str(rule_path), "strings_count": len(rule_strings)}
        )
        return {
            "rule_name": rule_name,
            "path": str(rule_path),
