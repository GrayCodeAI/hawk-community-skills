...
def detect_rapid_authentication(events, window_seconds=5, threshold=5):
    ...
    for user, user_evts in user_events.items():
        ...
        for _i, evt in enumerate(sorted_evts):
...
def main():
    ...
    parser.add_argument(
        "--max-events", type=int, default=2000, help="Max events to query (default: 2000)"
    )
    parser.add_argument(
        "--check-pipes", action="store_true", help="Also check Event 5145 for named pipe access"
    )
    parser.add_argument(
        "--check-smb-signing", action="store_true", help="Audit SMB signing on domain hosts"
    )
