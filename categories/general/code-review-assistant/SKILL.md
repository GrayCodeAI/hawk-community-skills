...
description: "Reviews code diffs in quick or deep modes with confidence-scored findings and optional fix application before a pull request."
license: MIT
tags:
- code-review
- pull-requests
...
1. **Load common.md** — the diff annotation algorithm, the size gate, the confidence rubric, what not to report, the output template, the fix-suggestion rules, and the data trust boundary. Both modes apply it in full.
...
3. **Pick the mode.** Default to quick and load quick-review.md. Load deep-review.md only when the user asks for depth or the change is risky or wide-reaching — it fans out by material, at higher cost.
4. **Load guidelines-audit.md** for the guideline-compliance portion, whichever mode ran.
