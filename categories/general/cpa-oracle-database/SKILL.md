---
name: cpa-oracle-database
description: "Skill: cpa-oracle-database"
license: MIT
tags: [general]
---

# az oracle-database

```bash
# Perform Lifecycle Management Action on Autonomous Database.
az oracle-database autonomous-database action

# Perform ChangeDisasterRecoveryConfiguration action on Autonomous Database.
az oracle-database autonomous-database change-disaster-recovery-configuration

# Create a AutonomousDatabase.
az oracle-database autonomous-database create

# Delete a AutonomousDatabase.
az oracle-database autonomous-database delete

# Perform failover action on Autonomous Database.
az oracle-database autonomous-database failover

# Generate wallet action on Autonomous Database.
az oracle-database autonomous-database generate-wallet

# List AutonomousDatabase resources by subscription ID.
az oracle-database autonomous-database list

# Restores an Autonomous Database based on the provided request parameters.
az oracle-database autonomous-database restore

# Get a AutonomousDatabase.
az oracle-database autonomous-database show

# This operation shrinks the current allocated storage down to the current actual used data storage.
az oracle-database autonomous-database shrink

# Perform switchover action on Autonomous Database.
az oracle-database autonomous-database switchover

# Update a AutonomousDatabase.
az oracle-database autonomous-database update

# Place the CLI in a waiting state until a condition is met.
az oracle-database autonomous-database wait

# Create a AutonomousDatabaseBackup.
az oracle-database autonomous-database backup create

# Delete a AutonomousDatabaseBackup.
az oracle-database autonomous-database backup delete

# List AutonomousDatabaseBackup resources by AutonomousDatabase.
az oracle-database autonomous-database backup list

# Get a AutonomousDatabaseBackup.
az oracle-database autonomous-database backup show

# Update a AutonomousDatabaseBackup.
az oracle-database autonomous-database backup update

# Place the CLI in a waiting state until a condition is met.
az oracle-database autonomous-database backup wait

# List AutonomousDatabaseCharacterSet resources by Location.
az oracle-database autonomous-database character-set list

# Get a AutonomousDatabaseCharacterSet.
az oracle-database autonomous-database character-set show

# List AutonomousDatabaseNationalCharacterSet resources by Location.
az oracle-database autonomous-database national-character-set list

# Get a AutonomousDatabaseNationalCharacterSet.
az oracle-database autonomous-database national-character-set show

# List AutonomousDbVersion resources by Location.
az oracle-database autonomous-database version list

# Get a AutonomousDbVersion.
az oracle-database autonomous-database version show

# Perform add storage capacity on exadata infra.
az oracle-database cloud-exadata-infrastructure add-storage-capacity

# Configures Exascale on Cloud exadata infrastructure resource.
az oracle-database cloud-exadata-infrastructure configure-exascale

# Create a CloudExadataInfrastructure.
az oracle-database cloud-exadata-infrastructure create

# Delete a CloudExadataInfrastructure.
az oracle-database cloud-exadata-infrastructure delete

# List CloudExadataInfrastructure resources by subscription ID.
az oracle-database cloud-exadata-infrastructure list

# Get a CloudExadataInfrastructure.
az oracle-database cloud-exadata-infrastructure show

# Update a CloudExadataInfrastructure.
az oracle-database cloud-exadata-infrastructure update

# Place the CLI in a waiting state until a condition is met.
az oracle-database cloud-exadata-infrastructure wait

# List DbServer resources by CloudExadataInfrastructure.
az oracle-database cloud-exadata-infrastructure database-server list

# Get a DbServer.
az oracle-database cloud-exadata-infrastructure database-server show

# Add VMs to the VM Cluster.
az oracle-database cloud-vm-cluster add-vm

# Create a CloudVmCluster.
az oracle-database cloud-vm-cluster create

# Delete a CloudVmCluster.
az oracle-database cloud-vm-cluster delete

# List CloudVmCluster resources by subscription ID.
az oracle-database cloud-vm-cluster list

# List Private IP Addresses by the provided filter.
az oracle-database cloud-vm-cluster list-private-ip-address

# Remove VMs from the VM Cluster.
az oracle-database cloud-vm-cluster remove-vm

# Get a CloudVmCluster.
az oracle-database cloud-vm-cluster show

# Update a CloudVmCluster.
az oracle-database cloud-vm-cluster update

# Place the CLI in a waiting state until a condition is met.
az oracle-database cloud-vm-cluster wait

# VM actions on DbNode of VM Cluster by the provided filter.
az oracle-database cloud-vm-cluster database-node action

# List DbNode resources by CloudVmCluster.
az oracle-database cloud-vm-cluster database-node list

# Get a DbNode.
az oracle-database cloud-vm-cluster database-node show

# Create a VirtualNetworkAddress.
az oracle-database cloud-vm-cluster virtual-network-address create

# Delete a VirtualNetworkAddress.
az oracle-database cloud-vm-cluster virtual-network-address delete

# List VirtualNetworkAddress resources by CloudVmCluster.
az oracle-database cloud-vm-cluster virtual-network-address list

# Get a VirtualNetworkAddress.
az oracle-database cloud-vm-cluster virtual-network-address show

# Update a VirtualNetworkAddress.
az oracle-database cloud-vm-cluster virtual-network-address update

# Place the CLI in a waiting state until a condition is met.
az oracle-database cloud-vm-cluster virtual-network-address wait

# List DbSystemShape resources by Location.
az oracle-database database-system-shape list

# Get a DbSystemShape.
az oracle-database database-system-shape show

# List DbVersion resources by SubscriptionLocationResource.
az oracle-database db-system-db-version list

# Get a DbVersion.
az oracle-database db-system-db-version show

# Create a DbSystem.
az oracle-database db-system create

# Delete a DbSystem.
az oracle-database db-system delete

# List DbSystem resources by subscription ID.
az oracle-database db-system list

# Get a DbSystem.
az oracle-database db-system show

# Update a DbSystem.
az oracle-database db-system update

# Place the CLI in a waiting state until a condition is met.
az oracle-database db-system wait

# List DnsPrivateView resources by Location.
az oracle-database dns-private-view list

# Get a DnsPrivateView.
az oracle-database dns-private-view show

# List DnsPrivateZone resources by Location.
az oracle-database dns-private-zone list

# Get a DnsPrivateZone.
az oracle-database dns-private-zone show

# Create a ExadbVmCluster.
az oracle-database exadb-vm-cluster create

# Delete a ExadbVmCluster.
az oracle-database exadb-vm-cluster delete

# List ExadbVmCluster resources by subscription ID.
az oracle-database exadb-vm-cluster list

# Remove VMs from the VM Cluster.
az oracle-database exadb-vm-cluster remove-vm

# Get a ExadbVmCluster.
az oracle-database exadb-vm-cluster show

# Update a ExadbVmCluster.
az oracle-database exadb-vm-cluster update

# Place the CLI in a waiting state until a condition is met.
az oracle-database exadb-vm-cluster wait

# VM actions on DbNode of ExadbVmCluster by the provided filter.
az oracle-database exadb-vm-cluster db-node action

# List ExascaleDbNode resources by ExadbVmCluster.
az oracle-database exadb-vm-cluster db-node list

# Get a ExascaleDbNode.
az oracle-database exadb-vm-cluster db-node show

# Create a ExascaleDbStorageVault.
az oracle-database exascale-db-storage-vault create

# Delete a ExascaleDbStorageVault.
az oracle-database exascale-db-storage-vault delete

# List ExascaleDbStorageVault resources by subscription ID.
az oracle-database exascale-db-storage-vault list

# Get a ExascaleDbStorageVault.
az oracle-database exascale-db-storage-vault show

# Update a ExascaleDbStorageVault.
az oracle-database exascale-db-storage-vault update

# Place the CLI in a waiting state until a condition is met.
az oracle-database exascale-db-storage-vault wait

# List FlexComponent resources by SubscriptionLocationResource.
az oracle-database flex-component list

# Get a FlexComponent.
az oracle-database flex-component show

# List GiMinorVersion resources by GiVersion.
az oracle-database gi-minor-version list

# Get a GiMinorVersion.
az oracle-database gi-minor-version show

# List GiVersion resources by Location.
az oracle-database gi-version list

# Get a GiVersion.
az oracle-database gi-version show

# Create a NetworkAnchor.
az oracle-database network-anchor create

# Delete a NetworkAnchor.
az oracle-database network-anchor delete

# List NetworkAnchor resources by subscription ID.
az oracle-database network-anchor list

# Get a NetworkAnchor.
az oracle-database network-anchor show

# Update a NetworkAnchor.
az oracle-database network-anchor update

# Place the CLI in a waiting state until a condition is met.
az oracle-database network-anchor wait

# Create a ResourceAnchor.
az oracle-database resource-anchor create

# Delete a ResourceAnchor.
az oracle-database resource-anchor delete

# List ResourceAnchor resources by subscription ID.
az oracle-database resource-anchor list

# Get a ResourceAnchor.
az oracle-database resource-anchor show

# Update a ResourceAnchor.
az oracle-database resource-anchor update

# Place the CLI in a waiting state until a condition is met.
az oracle-database resource-anchor wait

# List SystemVersion resources by Location.
az oracle-database system-version list

# Get a SystemVersion.
az oracle-database system-version show

# Create a VirtualNetworkAddress.
az oracle-database virtual-network-address create

# Delete a VirtualNetworkAddress.
az oracle-database virtual-network-address delete

# List VirtualNetworkAddress resources by CloudVmCluster.
az oracle-database virtual-network-address list

# Get a VirtualNetworkAddress.
az oracle-database virtual-network-address show

# Update a VirtualNetworkAddress.
az oracle-database virtual-network-address update

# Place the CLI in a waiting state until a condition is met.
az oracle-database virtual-network-address wait
```