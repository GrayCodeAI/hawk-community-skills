...
class SAMLSSOValidator:
    ...
    def _check_certificate_expiration(self):
        ...
        try:
            with open(self.config.certificate_path) as f:
                cert_pem = f.read()
    ...
    def _check_metadata_endpoint(self):
        ...
        try:
            req = urllib.request.Request(
                self.config.metadata_url, headers={"User-Agent": "SAML-SSO-Validator/1.0"}
            )
        ...
        try:
            ...
            for slo in root.findall(".//md:SingleLogoutService", self.SAML_NS):
                result["slo_urls"].append(
                    {"binding": slo.get("Binding", ""), "location": slo.get("Location", "")}
                )
    ...
    def generate_report(self) -> str:
        ...
        if failed:
            ...
            for r in sorted(
                failed, key=lambda x: {"critical": 0, "high": 1, "medium": 2, "low": 3}[x.severity]
            ):
