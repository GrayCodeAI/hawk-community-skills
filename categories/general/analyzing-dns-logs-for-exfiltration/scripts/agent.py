...
def detect_tunneling(dns_records, subdomain_len_threshold=50, min_queries=20):
    ...
    domain_stats = defaultdict(
        lambda: {"queries": 0, "unique_queries": set(), "subdomain_lengths": [], "sources": set()}
    )
...
def detect_txt_abuse(dns_records, threshold=100):
    ...
    for src, data in txt_counts.items():
        if data["count"] > threshold:
            level = (
                "CRITICAL" if data["count"] > 1000 else "HIGH" if data["count"] > 500 else "MEDIUM"
            )
            alerts.append(
                {
                    "src_ip": src,
...
if __name__ == "__main__":
    ...
    demo_records = (
        [
            {"query": f"{'a' * 60}.evil-tunnel.com", "src_ip": "192.168.1.105", "query_type": "TXT"}
            for _ in range(50)
        ]
        + [
            {"query": "x8kj2m9p4qw7nz3.xyz", "src_ip": "192.168.1.110", "query_type": "A"}
            for _ in range(5)
        ]
        + [{"query": "google.com", "src_ip": "192.168.1.50", "query_type": "A"} for _ in range(10)]
    )
