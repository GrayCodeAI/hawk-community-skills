...
def detect_impersonation_signals(text, known_sender_style=None):
    ...
    if re.search(
        r"(handle|process|complete).*(today|immediately|by end of day)", text, re.IGNORECASE
    ):
        signals.append("time_pressure")
    if known_sender_style:
        current = analyze_writing_style(text)
        diff = abs(
            current["mean_sentence_length"] - known_sender_style.get("mean_sentence_length", 15)
        )
        if diff > 8:
