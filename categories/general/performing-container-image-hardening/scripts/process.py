...
def check_non_root_user(config: dict) -> HardeningCheck:
    ...
    if user and user != "0" and user != "root":
        return HardeningCheck("CIS-4.1", "Non-root user configured", True, f"User: {user}", "HIGH")
...
def check_healthcheck(config: dict) -> HardeningCheck:
    ...
    if healthcheck and healthcheck.get("Test"):
        return HardeningCheck(
            "CIS-4.6", "HEALTHCHECK defined", True, f"Test: {healthcheck['Test']}", "MEDIUM"
        )
    return HardeningCheck(
        "CIS-4.6", "HEALTHCHECK defined", False, "No HEALTHCHECK instruction found.", "MEDIUM"
    )
...
def check_exposed_ports(config: dict) -> HardeningCheck:
    ...
    if len(port_list) <= 2:
        return HardeningCheck("HARD-1", "Minimal ports exposed", True, f"Ports: {port_list}", "LOW")
    return HardeningCheck(
        "HARD-1",
...
def check_image_size(image: str) -> HardeningCheck:
    ...
    try:
        ...
        return HardeningCheck(
            "HARD-2", "Image size within limits", False, "Could not determine image size", "LOW"
        )
...
def check_shell_available(image: str) -> HardeningCheck:
    ...
    try:
        ...
        return HardeningCheck(
            "HARD-3", "Shell removed from image", True, "No shell available in image", "LOW"
        )
