...
def generate_report(vulns: list, quality_gate: dict, snyk_json: dict, project_path: str) -> dict:
...
def main():
    ...
    parser.add_argument("--all-projects", action="store_true", help="Scan all projects in monorepo")
    parser.add_argument(
        "--monitor", action="store_true", help="Also run snyk monitor for continuous tracking"
    )
    ...
    for sev, count in sorted(
        quality_gate["severity_counts"].items(), key=lambda x: SEVERITY_ORDER.get(x[0], 3)
    ):
    ...
    if args.monitor:
        ...
        subprocess.run(
            ["snyk", "monitor", "--project-name", os.path.basename(project_path)], cwd=project_path
        )
