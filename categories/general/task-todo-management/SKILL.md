...
description: "Manage tasks and to-dos through a suite CLI: create, assign, track, complete, and organize tasks and task lists. Use when a user wants to manage personal or team to-dos."
license: MIT
tags:
- tasks
- todo
...
> - **走 lark-meeting 的 `minutes +todo`**（禁止本 skill）：上下文含 **妙记 / 会议纪要 / minute_token / 妙记 URL**（`/minutes/`）；或「在某某妙记里新建/修改待办」「妙记 AI 待办」「会议录制里的待办」。
