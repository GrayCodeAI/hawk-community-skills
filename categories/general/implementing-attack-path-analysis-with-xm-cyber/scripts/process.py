...
class AttackPathAnalyzer:
    ...
    def load_attack_paths(self, data):
        ...
        print(
            f"[+] Loaded {self.graph.number_of_nodes()} nodes, {self.graph.number_of_edges()} edges"
        )
        ...
        for critical_asset in self.critical_assets:
            ...
            for source in self.graph.nodes():
                ...
                if source in self.critical_assets:
                    continue
                try:
                    for path in nx.all_simple_paths(self.graph, source, critical_asset, cutoff=8):
            ...
            self.choke_points.append(
                {
                    ...
                    "risk_score": round(
                        counts["paths"] * len(counts["assets"]) * (1 + betweenness.get(node_id, 0)),
                        2,
                    ),
    ...
    def print_summary(self):
        ...
        if self.choke_points:
            ...
            for i, cp in enumerate(self.choke_points[:10], 1):
                ...
                print(f"     Type: {cp['entity_type']} | Category: {cp['exposure_category']}")
                print(
