...
class BeaconAnalyzer:
    ...
    def _extract_manual(self, filepath):
        ...
        for xor_key in [0x2E, 0x69]:
            ...
            magic = bytes([0x00 ^ xor_key, 0x01 ^ xor_key, 0x00 ^ xor_key, 0x02 ^ xor_key])
...
def main():
    parser = argparse.ArgumentParser(description="Cobalt Strike Beacon Configuration Analyzer")
    parser.add_argument("--file", help="Single file to analyze")
    parser.add_argument("--directory", help="Directory for batch analysis")
    parser.add_argument("--output", default="beacon_report.json", help="Output report path")
    parser.add_argument("--scan-memory", action="store_true", help="Treat input as raw memory dump")
