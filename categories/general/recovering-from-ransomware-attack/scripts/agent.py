...
class RansomwareRecoveryAgent:
            ...
            systems.append(
                {
                    ...
                    "status": "pending",
                    "recovery_method": (
                        "Restore from backup" if clean_backup_available else "Rebuild from scratch"
                    ),
                }
            )
    ...
    def generate_credential_reset_checklist(self):
        ...
        checklist = [
            ...
            {"item": "Reset all user passwords", "status": "pending", "priority": "high"},
            {
                "item": "Revoke and reissue all certificates",
                "status": "pending",
                "priority": "high",
            },
            {"item": "Rotate all API keys and tokens", "status": "pending", "priority": "high"},
            {"item": "Reset cloud IAM credentials", "status": "pending", "priority": "high"},
            {
    ...
    def get_recovery_progress(self):
        ...
        completed = sum(
            1 for s in self.recovery["systems"] if s["status"] in ("recovered", "verified")
        )
        return {
            "total_systems": total,
            "recovered": completed,
