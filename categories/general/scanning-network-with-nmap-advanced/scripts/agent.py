...
def scan_ports(scanner, hosts, ports="1-1024", timing="T4"):
    ...
    scanner.scan(
        hosts=target_str, ports=ports, arguments=f"-sS -{timing} --min-rate 3000 --max-retries 2"
    )
    for host in scanner.all_hosts():
        results[host] = []
...
def service_version_scan(scanner, host, open_ports):
    ...
    scanner.scan(
        hosts=host, ports=port_str, arguments="-sV --version-intensity 5 -sC -O --osscan-guess"
    )
...
def generate_report(discovery, port_results, version_info, vulnerabilities, output_dir):
    ...
    with open(csv_path, "w", newline="") as f:
        ...
        for host, ports in port_results.items():
            for p in ports:
                writer.writerow(
                    [host, p["port"], p["protocol"], p["service"], p["product"], p["version"]]
                )
