...
def main():
    ...
    modules = (
        args.modules.split(",")
        if args.modules != "all"
        else ["dns", "subdomains", "whois", "emails", "tech", "dorks", "security_txt", "robots_txt"]
    )
    ...
    with Progress(
        ...
        if "tech" in modules:
            ...
            progress.update(
                task, completed=True, description="[green]Technology fingerprint complete"
            )
    ...
    table.add_row(
        "Emails Discovered", str(len(results.get("emails", {}).get("discovered_emails", [])))
    )
