...
class NiktoScanner:
    ...
    def scan_target(
        self, target: str, tuning: str = "123456789abc", pause: int = 1, ssl: bool = False
    ) -> dict:
    ...
    def scan_targets(self, targets: list, max_parallel: int = 3, **kwargs) -> list:
        ...
        with ThreadPoolExecutor(max_workers=max_parallel) as executor:
            futures = {
                executor.submit(self.scan_target, target, **kwargs): target for target in targets
            }
            ...
            for scan_details in root.findall(".//scandetails"):
                ...
                for item in scan_details.findall("item"):
                    ...
                    if any(
                        ...
                        w in desc_lower
                        for w in ["sql injection", "xss", "cross-site", "file inclusion"]
                    ):
                        finding["severity"] = "High"
                    elif any(
                        w in desc_lower
                        for w in ["directory listing", "information disclosure", "version"]
                    ):
