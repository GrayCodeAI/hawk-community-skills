...
def defang_ioc(value, ioc_type):
    ...
    if ioc_type == "ipv4" or ioc_type in ("domain", "email"):
        return value.replace(".", "[.]")
    elif ioc_type == "url":
        return (
            value.replace("http://", "hxxp://").replace("https://", "hxxps://").replace(".", "[.]")
        )
    return value
...
def refang_ioc(value, ioc_type):
    ...
    if ioc_type in ("ipv4", "domain", "email"):
        ...
        return (
            value.replace("hxxp://", "http://").replace("hxxps://", "https://").replace("[.]", ".")
        )
    return value
...
def main():
    ...
    parser.add_argument(
        "--abuseipdb-key", default=os.environ.get("ABUSEIPDB_KEY", ""), help="AbuseIPDB API key"
    )
