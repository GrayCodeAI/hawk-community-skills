---
name: vector-animation-playback
description: "Implement vector animations in web projects with runtime playback, web workers, state machines, theming, and performance best practices."
license: MIT
tags:
- animation
- playback
...
```typescript
dotLottie.addEventListener('load', () => console.log('Animation loaded'));
dotLottie.addEventListener('play', () => console.log('Playing'));
dotLottie.addEventListener('complete', () => console.log('Animation completed'));
dotLottie.addEventListener('frame', ({ currentFrame }) => console.log('Frame:', currentFrame));

// Clean up (use a named handler to remove later)
...
See the [Web Workers](#web-workers-recommended-for-performance) section above. Offload complex or multiple animations to worker threads with `DotLottieWorker`.
...
DotLottie automatically freezes animations when they're not visible (offscreen). To disable this behavior, or tune other `renderConfig` options:

```typescript
const dotLottie = new DotLottie({
  canvas,
  src: 'animation.lottie',
  useFrameInterpolation: true,  // Smooth playback (default); false matches AE frame rate
  renderConfig: {
    freezeOnOffscreen: false,        // Disable auto-freeze (not recommended)
    devicePixelRatio: 0.75 * window.devicePixelRatio, // Default 75% for perf
    autoResize: true,                // Canvas resizes to fit its container
  },
});
```

- `devicePixelRatio: window.devicePixelRatio` gives full retina quality at higher CPU/GPU cost (default is 75%).
- Set canvas size via CSS (`<canvas style="width:400px;height:400px">`) and DotLottie auto-determines the optimal drawing area; use `autoResize: true` to follow container changes.

### 4. Clean Up (Vanilla JS only)
...
Use the `autoResize` render config (see [Performance Best Practices](#performance-best-practices)) to automatically resize when the container changes.
