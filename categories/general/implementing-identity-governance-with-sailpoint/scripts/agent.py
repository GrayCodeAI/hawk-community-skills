...
def list_identities(base_url, token, limit=250):
    ...
    return sailpoint_api(base_url, token, "/v3/search/identities", params={"limit": limit})
...
def audit_sod_violations(sod_data):
    ...
    violations = sod_data if isinstance(sod_data, list) else sod_data.get("violations", [])
...
def audit_orphan_accounts(accounts_data):
    ...
    accounts = (
        accounts_data if isinstance(accounts_data, list) else accounts_data.get("accounts", [])
    )
    orphans = []
...
def audit_access_reviews(reviews_path):
    ...
    for review in items:
        ...
        unused = [
            e for e in entitlements if not e.get("used_last_90_days", e.get("last_used", "") != "")
        ]
        if len(unused) > 3:
