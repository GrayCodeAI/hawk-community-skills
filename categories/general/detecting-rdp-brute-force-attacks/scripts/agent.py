...
class RDPBruteForceDetector:
    ...
    def __init__(
        self, evtx_path, threshold=10, time_window_minutes=15, output_dir="./rdp_brute_force_report"
    ):
    ...
    def analyze(self):
        ...
        substatus_counter = Counter(
            FAILURE_SUBSTATUS.get(e["sub_status"], e["sub_status"]) for e in self.failed_logons
        )

        brute_force_ips = {
            ip: cnt for ip, cnt in ip_counter.items() if cnt >= self.threshold and ip != "-"
        }
        ...
        for ip in brute_force_ips:
            if ip in success_ips:
                user_list = [
                    e["target_user"] for e in self.successful_logons if e["source_ip"] == ip
                ]
                compromised.append(
                    {
                        "ip": ip,
        ...
        spray_ips = {ip: len(users) for ip, users in users_per_ip.items() if len(users) >= 5}
...
def main():
    ...
    parser.add_argument(
        "--output-dir", default="./rdp_brute_force_report", help="Output directory for report"
    )
    args = parser.parse_args()

    os.makedirs(args.output_dir, exist_ok=True)
    detector = RDPBruteForceDetector(args.evtx_file, args.threshold, output_dir=args.output_dir)
