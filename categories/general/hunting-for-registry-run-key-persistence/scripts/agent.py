...
def is_run_key(target_object):
    ...
    return any(rk.lower() in target_lower for rk in RUN_KEY_PATHS)
...
def analyze_sysmon_events(events):
    ...
    for evt in events:
        ...
        if "powershell" in details_lower and (
            "-enc" in details_lower or "-e " in details_lower or "encodedcommand" in details_lower
        ):
            severity = "critical"
    ...
    for entry in entries:
        ...
        for lolbin in LOLBINS:
            if lolbin.lower() in data_lower:
                ...
                indicators.append(f"lolbin:{lolbin}")
                break
        if Path(value_data.strip('"').split(" ")[0]).suffix.lower() not in (
            ".exe",
            ".dll",
            ".bat",
            ".cmd",
...
def main():
    ...
    ap.add_argument(
        "--input", required=True, help="Input JSON with Sysmon events or registry snapshot"
    )
