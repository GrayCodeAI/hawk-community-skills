...
def is_admin_sid(sid: str, domain_sid: str) -> bool:
    if sid in ADMIN_SIDS:
        return True
    return any(sid == domain_sid + suffix for suffix in ADMIN_RID_SUFFIXES)
...
def analyze_acls(dc_ip: str, domain: str, username: str, password: str, target_ou: str) -> dict:
    ...
    for entry in conn.entries:
        ...
        obj_classes = (
            [str(c) for c in entry.objectClass.values] if hasattr(entry, "objectClass") else []
        )
        obj_type = "unknown"
...
def main():
    ...
    parser.add_argument(
        "--target-ou", default=None, help="Target OU distinguished name to scope the search"
    )
    parser.add_argument("--output", default=None, help="Output JSON file path")
    args = parser.parse_args()

    result = analyze_acls(args.dc_ip, args.domain, args.username, args.password, args.target_ou)
