    ...
    for pattern in ransom_note_patterns:
        for match in root.rglob(pattern):
            if match.is_file() and match.stat().st_size < 1_000_000:
                with open(match, errors="ignore") as f:
                    content = f.read(4096)
                found_notes.append(
                    {
                        "path": str(match),
...
def parse_windows_event_logs(evtx_export_path: str) -> list[dict]:
    ...
    with open(evtx_export_path, newline="", errors="ignore") as f:
...
if __name__ == "__main__":
    ...
    with open(output, "w") as f:
        json.dump(
            {"ransom_notes": notes, "encrypted_files": encrypted, "iocs": iocs, "events": events},
            f,
            indent=2,
        )
