...
class ServiceAccountAuditor:
    ...
    def _check_privilege_levels(self):
        over_priv = [
            a for a in self.accounts if a.member_of_privileged_group and a.status == "active"
        ]
        if over_priv:
            self.findings.append(
                AuditFinding(
                    ...
                    affected_accounts=[
                        f"{a.username}@{a.platform} ({a.privilege_level})" for a in over_priv
                    ],
    ...
    def _check_interactive_logon(self):
        interactive = [
            a for a in self.accounts if a.interactive_logon_allowed and a.status == "active"
        ]
        if interactive:
            self.findings.append(
                AuditFinding(
