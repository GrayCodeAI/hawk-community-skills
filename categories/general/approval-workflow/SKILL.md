...
description: "Handle approval workflows through a suite CLI: query pending approvals, review instances, and approve, reject, transfer, or initiate them. Use when a user wants to process approvals."
license: MIT
tags:
- approval
- workflow
...
**判定规则：** 只要最终动作是对审批单据做同意、拒绝、转交、退回、撤回、催办、加签、抄送、查详情、查已发起/已办/待办，就归 `lark-approval`。只有当用户处理的是**非审批类任务/待办**时，才走 `lark-task`。
...
| 查/搜待办、已办 | `tasks query`（`topic`：1待办 2已办 17未读 18已读） | `lark-approval-tasks-query.md`       |
...
| 按定义/关键词查已发起审批 | `instances initiated` | `lark-approval-instances-initiated.md` |
