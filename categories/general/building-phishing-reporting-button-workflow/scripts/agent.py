...
def analyze_email(parsed):
    ...
    for pat in SUSPICIOUS_PATTERNS:
        ...
        if matches:
            findings.append(
                {"indicator": "Suspicious language", "detail": matches[0][:100], "weight": 10}
            )
            score += 10
    ...
    if suspicious_urls:
        findings.append(
            {"indicator": "Suspicious URLs", "detail": suspicious_urls[:5], "weight": 15}
        )
    ...
    for att in parsed.get("attachments", []):
        ...
        if any(
            fn.endswith(ext)
            for ext in [".exe", ".scr", ".js", ".vbs", ".bat", ".ps1", ".hta", ".iso", ".img"]
        ):
            findings.append(
                {"indicator": "Dangerous attachment", "detail": att["filename"], "weight": 30}
            )
            score += 30
