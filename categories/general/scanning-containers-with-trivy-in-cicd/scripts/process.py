...
def main():
    ...
    parser.add_argument(
        "--fail-on-findings", action="store_true", help="Exit non-zero if quality gate fails"
    )
    parser.add_argument(
        "--ignore-unfixed",
        action="store_true",
    ...
    parser.add_argument(
        "--dockerfile", default=None, help="Path to Dockerfile for misconfiguration scanning"
    )
    ...
    report = {
        ...
        "vulnerabilities": [
            ...
            for v in sorted(
                scan_result.vulnerabilities, key=lambda x: SEVERITY_ORDER.get(x.severity, 4)
            )
        ],
        "misconfigurations": [
            {
