...
def authenticate(base_url, username, password, auth_method="CyberArk"):
    ...
    resp = requests.post(url, json=payload, verify=True, timeout=30)
...
def audit_accounts_standing_access(base_url, headers):
    ...
    resp = requests.get(url, headers=headers, params=params, verify=True, timeout=30)
    resp.raise_for_status()
...
def main():
    ...
    parser.add_argument(
        "action", choices=["safes", "audit-safe", "platforms", "sessions", "accounts", "full-audit"]
    )
