...
def audit_standing_privileges(privileges_path):
    ...
    for priv in privs:
        ...
        is_privileged = any(
            k in role for k in ["admin", "root", "owner", "superuser", "dba", "operator"]
        )
...
def calculate_jit_metrics(requests_path):
    ...
    return {
        ...
        "avg_approval_time_min": round(sum(approval_times) / len(approval_times), 1)
        if approval_times
        else 0,
        ...
        "p95_approval_time_min": sorted(approval_times)[int(len(approval_times) * 0.95)]
        if approval_times
        else 0,
    }
...
def main():
    ...
    parser.add_argument(
        "--action", choices=["audit", "standing", "metrics", "policy", "full"], default="full"
    )
