    ...
    try:
        with open(ssl_log_path) as f:
            for line in f:
                ...
                if len(fields) < 22:
                    continue
                ts, _uid, src_ip, _src_port, dst_ip, dst_port = (
                    fields[0],
                    fields[1],
                    fields[2],
                    fields[3],
...
def main():
    ...
    parser.add_argument(
        "--min-connections", type=int, default=20, help="Minimum connections for beacon analysis"
    )
    parser.add_argument(
        "--max-jitter", type=int, default=25, help="Maximum jitter percentage for beacon scoring"
    )
