...
def parse_hashcat_status(potfile):
    ...
    try:
        for line in (
            Path(potfile).read_text(encoding="utf-8", errors="replace").strip().splitlines()
        ):
            parts = line.split(":", 1)
            if len(parts) == 2:
    ...
    return {
        ...
        "top_passwords": [
            p for p, _ in __import__("collections").Counter(passwords).most_common(10)
        ],
    }
