...
class AlertAnalyzer:
    ...
    def add_alert(
        self, rule_name: str, classification: str, source: str, severity: str, triage_time_sec: int
    ):
        self.alerts.append(
            {
                "rule_name": rule_name,
    ...
    def get_overall_metrics(self) -> dict:
        ...
        return {
            ...
            "total_analyst_hours_wasted_on_fp": round(
                sum(a["triage_time_sec"] for a in self.alerts if a["classification"] == "fp")
                / 3600,
                1,
            ),
        }
...
if __name__ == "__main__":
    ...
    for rule_name, fp_prob in rules:
        for _ in range(random.randint(20, 80)):
            classification = (
                "fp" if random.random() < fp_prob else ("tp" if random.random() > 0.3 else "btp")
            )
            analyzer.add_alert(
                rule_name,
                classification,
