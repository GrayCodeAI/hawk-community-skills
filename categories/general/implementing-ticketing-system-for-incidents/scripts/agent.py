...
class ServiceNowClient:
    ...
    def get_incident(self, number):
        ...
        resp = self.session.get(
            f"{self.base_url}/table/incident", params={"sysparm_query": f"number={number}"}
        )
        resp.raise_for_status()
...
class TheHiveClient:
    ...
    def create_task(self, case_id, title, description="", group="default"):
        ...
        data = {"title": title, "description": description, "group": group, "status": "Waiting"}
    ...
    def list_cases(self, status="Open", limit=50):
        ...
        resp = self.session.post(
            f"{self.base_url}/api/case/_search", json=query, params={"range": f"0-{limit}"}
        )
        resp.raise_for_status()
        return resp.json()
...
def calculate_sla_metrics(incidents):
    ...
    metrics["sla_compliance_pct"] = round(metrics["within_sla"] / max(metrics["total"], 1) * 100, 1)
    return metrics
