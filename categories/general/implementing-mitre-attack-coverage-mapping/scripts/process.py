...
class DetectionRule:
    ...
    def __init__(
        self, name: str, techniques: list, score: int, data_sources: list, validated: bool = False
    ):
        self.name = name
...
class CoverageMap:
    ...
    def get_coverage_summary(self) -> dict:
        ...
        scores = [
            self.technique_scores.get(t, {}).get("max_score", 0) for t in ENTERPRISE_TECHNIQUES
        ]
    ...
    def get_tactic_coverage(self) -> dict:
        ...
        for tactic_id, tactic_name in ATTACK_TACTICS.items():
            techniques_in_tactic = [
                t for t, info in ENTERPRISE_TECHNIQUES.items() if info["tactic"] == tactic_id
            ]
            covered = sum(
                1
    ...
    def generate_navigator_layer(self) -> dict:
        techniques = []
        for tech_id, _info in ENTERPRISE_TECHNIQUES.items():
            score = self.technique_scores.get(tech_id, {}).get("max_score", 0)
            ...
            techniques.append(
                {
                    ...
                    "color": color,
                    "score": score,
                    "comment": f"{len(rules)} rules: {', '.join(rules[:3])}"
                    if rules
                    else "NO DETECTION",
                }
            )
...
if __name__ == "__main__":
    ...
    rules = [
        ...
        DetectionRule(
            "Lateral Movement RDP", ["T1021"], 70, ["Windows Security Log", "Firewall"], True
        ),
