---
name: agent-skill-authoring-skill-creator
description: "Create and update agent skills that extend capabilities with specialized knowledge, workflows, and bundled resources."
license: MIT
tags:
- skills
- agents
...
- [ ] `vally eval` passes (no error-severity findings) when `COPILOT_TOKEN` is available; otherwise lint-only is acceptable, matching the `Vally Evaluation` workflow behavior
