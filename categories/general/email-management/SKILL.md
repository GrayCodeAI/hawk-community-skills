...
description: "Manage email through a collaboration-suite CLI: read, search, compose, send, reply, forward, and organize mailbox messages. Use when a user wants to handle email in their work mailbox."
license: MIT
tags:
- email
- messaging
...
8. **HTML body 预检（可选）** — 复杂 HTML body 提交前可先跑 `+lint-html` 看 lint 会改 / 删什么；写信路径（`+send` / `+draft-create` / `+reply` / `+reply-all` / `+forward` / `+draft-edit` body op）已内置 autofix，普通正文不必先跑。详见 references/lark-mail-html.md 中的「写入路径内置 HTML lint」章节
...
- 收件人地址搜索：搜索用户邮箱地址、群邮箱地址、邮件组地址，提供给用户确认。ref: lark-mail-recipient-search
- 使用公共邮箱发信、使用邮箱别名发信：通过 `--mailbox` 指定邮箱归属，通过 `--from` 指定发件人地址。ref: lark-mail-send-as
- 查看发送邮件后的投递状态：发送成功后查看邮件投递状态；也覆盖发送拦截。ref: lark-mail-send-status
- 使用邮件模板：区分个人模板和静态 HTML 模板，发信类 shortcut 用 `--template-id` 套用模板。ref: lark-mail-template
- 撤回已发送邮件：撤回邮件并查询异步撤回状态。ref: lark-mail-recall
- 修改邮件标签/已读状态/文件夹：优先使用 `+message-modify`。ref: `+message-modify`
- 软删除邮件：优先使用 `+message-trash`。ref: `+message-trash`
- 收信规则：查看、创建、更新、删除、启停、排序自动处理收到邮件的规则。ref: lark-mail-rules
- 分享邮件到 IM：分享邮件或会话到群聊、个人会话。ref: lark-mail-share-to-chat
- 发送日程邀请邮件：在邮件中嵌入 `text/calendar` 日程邀请。ref: lark-mail-calendar-invite
- 编写复杂 HTML 正文：复杂 HTML、本地图片、安全不确定时读取规范或运行 `+lint-html`；普通正文无需预读。ref: lark-mail-html
- 读取邮件：按场景选择 triage、单封、批量或会话读取。ref: `+triage`、`+message`、`+messages`、`+thread`
...
- 当需要查找收件人邮箱地址时，使用联系人搜索接口。ref: lark-mail-recipient-search
- **发送前必须向用户确认收件人和内容；如有必要，可引导用户去飞书邮件里打开草稿查看详情；用户明确同意后才可执行发送或使用 `--confirm-send`**
- **发送后必须调用 `send_status` 确认投递状态**；定时发送（`--send-time`）在预定发送时间后再查询。ref: lark-mail-send-status
- 公共邮箱/别名发信见 lark-mail-send-as
- 发送拦截见 lark-mail-send-status
...
- 简单正文直接使用常规 `<p>` / `<ul><li>`；复杂 HTML、本地图片或安全不确定时再读取 邮件 HTML 写法规范 或使用 `+lint-html`
- **官方模板库** `assets/templates/` 可供参考
