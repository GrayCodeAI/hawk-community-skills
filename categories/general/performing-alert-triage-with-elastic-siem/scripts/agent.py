...
def get_open_alerts(
    ...
    response = es.search(
        index=SIEM_SIGNALS_INDEX, query=query, size=size, sort=[{"@timestamp": {"order": "desc"}}]
    )
    alerts = []
...
def prioritize_alerts(alerts: list[dict]) -> list[dict]:
    ...
    return sorted(
        alerts,
        key=lambda a: (SEVERITY_PRIORITY.get(a.get("severity", "low"), 5), -a.get("risk_score", 0)),
    )
...
def identify_alert_clusters(alerts: list[dict]) -> list[dict]:
    ...
    for host, host_alerts in by_host.items():
        if len(host_alerts) >= 3:
            ...
            max_severity = min(
                host_alerts, key=lambda a: SEVERITY_PRIORITY.get(a.get("severity", "low"), 5)
            )
            clusters.append(
                {
                    "host": host,
