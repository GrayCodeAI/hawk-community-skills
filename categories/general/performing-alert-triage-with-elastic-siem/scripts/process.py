...
class TriageAlert:
    ...
    def calculate_triage_priority(self) -> dict:
        ...
        composite_score = min(
            100,
            (base_score * asset_multiplier * 0.4) + (tactic_score * 0.3) + chain_boost + ti_boost,
        )
...
class TriageQueue:
    ...
    def triage_alert(
        self, alert_id: str, classification: str, notes: str, triage_duration_seconds: int
    ):
    ...
    def get_triage_metrics(self) -> dict:
        ...
        return {
            ...
            "false_positive_rate": round(self.metrics["false_positives"] / total * 100, 1)
            if total > 0
            else 0,
...
if __name__ == "__main__":
    ...
    queue.triage_alert(
        "ALT-001", "true_positive", "Brute force followed by successful admin login", 300
    )
    queue.triage_alert(
        "ALT-003", "false_positive", "DNS queries to known CDN - normal behavior", 120
    )
    queue.triage_alert(
        "ALT-004", "benign_true_positive", "SCCM deployment creating expected service", 90
    )
