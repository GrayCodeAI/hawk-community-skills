...
class WazuhDetectionAgent:
    ...
    def get_rules_summary(self):
        ...
        data = self._api(
            "GET", "/rules", params={"limit": 500, "select": "id,description,level,groups"}
        )
        if not data:
            return {"total": 0}
    ...
    def check_vulnerability_detection(self):
        ...
        data = self._api(
            "GET", "/manager/configuration", params={"section": "vulnerability-detector"}
        )
        if data:
...
def main():
    parser = argparse.ArgumentParser(description="Manage Wazuh endpoint detection and query alerts")
