    ...
    for subdomain in subdomains:
        try:
            result = subprocess.run(
                ["dig", "+short", "CNAME", subdomain], capture_output=True, text=True, timeout=10
            )
...
def generate_report(
    domain: str, subdomains: list, live_hosts: list, takeover_candidates: list, output_dir: str
) -> str:
...
def main():
    ...
    domain = sys.argv[1]
    output_dir = (
        sys.argv[2] if len(sys.argv) > 2 and not sys.argv[2].startswith("--") else "./recon_output"
    )
