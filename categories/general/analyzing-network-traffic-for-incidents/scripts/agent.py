...
try:
    from scapy.all import ARP, DNS, DNSQR, IP, TCP, UDP, Raw, rdpcap
...
def detect_lateral_movement(pcap_path):
    ...
    connections = run_tshark(
        pcap_path, "tcp.flags.syn==1 && tcp.flags.ack==0", ["ip.src", "ip.dst", "tcp.dstport"]
    )
    lateral = []
...
def extract_dns_queries(pcap_path):
    ...
    queries = run_tshark(pcap_path, "dns.qr==0", ["ip.src", "dns.qry.name", "dns.qry.type"])
    return queries
