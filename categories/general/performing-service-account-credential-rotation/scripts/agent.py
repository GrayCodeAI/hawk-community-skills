...
class CredentialRotationAgent:
    ...
    def rotate_aws_access_key(self, username):
        ...
        try:
            ...
            self._log("AWS", username, "rotate-key", "success", {"new_key_id": new_key_id})
    ...
    def rotate_vault_database_creds(self, role_name):
        ...
        try:
            ...
            self._log("Vault", role_name, "generate-creds", "failed", {"status": resp.status_code})
    ...
    def verify_service_health(self, endpoints):
        ...
        for ep in endpoints:
            for attempt in range(3):
                try:
                    resp = requests.get(ep["url"], timeout=10, headers=ep.get("headers", {}))
