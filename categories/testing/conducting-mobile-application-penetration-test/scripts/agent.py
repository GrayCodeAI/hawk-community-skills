...
def scan_hardcoded_secrets(source_dir):
    ...
    patterns = {
        "API Key": re.compile(
            r'["\'](?:api[_-]?key|apikey)["\']?\s*[:=]\s*["\']([^"\']{20,})["\']', re.I
        ),
        "AWS Key": re.compile(r"AKIA[0-9A-Z]{16}"),
        "Private Key": re.compile(r"-----BEGIN (?:RSA )?PRIVATE KEY-----"),
        "Password": re.compile(
            r'["\'](?:password|passwd|pwd)["\']?\s*[:=]\s*["\']([^"\']+)["\']', re.I
        ),
...
def check_ssl_pinning(package_name):
    ...
    try:
        ...
        return {
            "ssl_pinning": "enabled" if "error" not in result.stdout.lower() else "not_detected"
        }
    except FileNotFoundError:
...
def run_pentest(apk_path):
    ...
    print(f"\n{'=' * 60}")
    print("  MOBILE APP PENETRATION TEST")
