...
class HostHeaderInjectionAgent:
    ...
    def _request(self, method, path, headers=None, data=None, timeout=10, allow_redirects=False):
        if not requests:
            return None
    ...
    def test_password_reset_poisoning(self, reset_path="/forgot-password", email="test@target.com"):
        ...
        for headers in payloads:
            resp = self._request("POST", reset_path, headers=headers, data={"email": email})
    ...
    def test_cache_poisoning(self, path="/"):
        ...
        self._request("GET", f"{path}{cache_buster}", headers={"X-Forwarded-Host": evil_host})
    ...
    def test_port_injection(self, path="/"):
        ...
        resp = self._request("GET", path, headers={"Host": "target.com:@evil.attacker.com"})
