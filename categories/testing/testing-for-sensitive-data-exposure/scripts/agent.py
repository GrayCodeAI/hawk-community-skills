...
def scan_javascript_files(base_url):
    ...
    findings = []
    try:
        resp = requests.get(base_url, timeout=15, verify=True)
        ...
        for js_path in js_urls[:20]:
            if js_path.startswith("//"):
                ...
                js_url = urljoin(base_url, js_path)
            try:
                js_resp = requests.get(js_url, timeout=15, verify=True)
...
def check_tls_config(host):
    ...
    findings = []
    try:
        resp = requests.get(f"http://{host}/", timeout=5, allow_redirects=False, verify=True)
...
def check_error_verbosity(base_url):
    ...
    for tr in test_requests:
        ...
        try:
            resp = requests.request(tr["method"], url, data=tr.get("data"), timeout=10, verify=True)
...
def main():
    ...
    parser.add_argument(
        "--endpoints", nargs="+", default=["/api/users/me", "/api/users", "/api/account"]
    )
